...
switch designtype
    ...
    case 'heat_kernel'
        t0=linspace(3,10,Nscales);
        t=0.0005624*1.44.^t0;
        Nscales = length(t);
        for j=1:Nscales
            g{j}= @(x) sgwt_kernel_heat(t(j)*x);
        end
        
    case 'wave_kernel' 
        lmin = 9.232885843227281;
        M = 30;
        lmin = lmax/M;
        delta=linspace(log(lmin),log(lmax),M);  
        single_delta = (delta(2)-delta(1));
        sigma=3*single_delta;
        t=linspace(log(lmin)+2*single_delta,log(lmax)-2*single_delta,Nscales+1);  

        for j=1:Nscales
            g{j}= @(x) sgwt_kernel_wave(x,t(j),sigma);
        end
...
function r = sgwt_kernel_heat(x)
  r=zeros(size(x));
  r=exp(-x);

end

function r = sgwt_kernel_wave(x,e,sigma)
  r=zeros(size(x));

  r=exp((-(e - log(x)).^2) ./ (2*sigma.^2));
  C = 1;%sum(exp((-(e-x).^2)/(2*sigma.^2)));
  r=r./C;
